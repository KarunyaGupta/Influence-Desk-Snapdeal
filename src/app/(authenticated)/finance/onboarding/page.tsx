...
const STATUS_FILTERS: { label: string; value: OnboardingRequestStatus | "all" }[] = [
  { label: "All", value: "all" },
  { label: "Needs Review", value: "pending_finance_review" },
  { label: "On Hold", value: "on_hold_finance" },
...
export default function FmOnboardingQueuePage() {
  ...
  useEffect(() => {
    // FM sees pending_finance_review + approved + rejected (not pending_bm_review)
    onboarding.listOnboardingRequests().then((list) => {
      setRequests(list.filter((r) => r.status !== "pending_bm_review" && r.status !== "on_hold_bm"));
    }).catch(() => {}).finally(() => setLoading(false));
  }, [onboarding]);

  const pendingCount = requests.filter((r) => r.status === "pending_finance_review").length;
  ...
  const columns: ColumnDef<OnboardingRequest>[] = [
    ...
    {
      key: "bmApproved", header: "BM Approved",
      render: (row) => <span className="text-sm text-muted-foreground">{row.bmReviewedAt ? formatDate(row.bmReviewedAt) : "—"}</span>,
    },
    {
  ...
  return (
    <div className="space-y-5">
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={CheckCircle2} heading="Nothing needs your attention" description="No onboarding requests match your filters." />
      ) : (
        <DataTable columns={columns} data={filtered} rowKey={(r) => r.id} onRowClick={(r) => router.push(`/finance/onboarding/${r.id}`)} />
      )}
    </div>
  );
}
